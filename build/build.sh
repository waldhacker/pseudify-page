...
if [ ! -x "$(command -v yarn)" ]; then echo "Error: docker is not installed."; exit 1; fi
...
function build
{
    ...
    ln -s "$CURRENT_DOCS_VERSION" latest
    cd "$_ROOT_DIRECTORY/dist/docs/latest/"
    sed -i 's#/docs/[0-9]\+\.[0-9]\+/#/docs/latest/#g' sitemap.xml
    gzip -kf sitemap.xml
}
